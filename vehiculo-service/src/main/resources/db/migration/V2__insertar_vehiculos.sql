INSERT INTO vehiculo
(patente, marca, modelo, anio, color, tipo_vehiculo)
VALUES
    ('ABCD12', 'Toyota', 'Corolla', 2022, 'Rojo', 'Sedan'),
    ('EFGH34', 'Hyundai', 'Accent', 2021, 'Blanco', 'Sedan'),
    ('IJKL56', 'Kia', 'Sportage', 2023, 'Negro', 'SUV'),
    ('MNOP78', 'Chevrolet', 'Spark', 2020, 'Azul', 'Hatchback'),
    ('QRST90', 'Nissan', 'Versa', 2022, 'Gris', 'Sedan'),
    ('UVWX11', 'Mazda', 'CX-5', 2024, 'Plateado', 'SUV'),
    ('YZAB22', 'Suzuki', 'Swift', 2021, 'Rojo', 'Hatchback'),
    ('CDEF33', 'Ford', 'Ranger', 2023, 'Blanco', 'Camioneta');